...
const App: FC = () => {
  ...
  const {
    ...
    activeEmployeesList,
    isOfflineMode,
  }: EmployeesSliceState = useAppSelector(employeesSlice.selectors.employees);

  const setActiveUsers = useCallback(() => {
    if (activeEmployeesList.length) localStorage.setItem(localStorageKeys.activeUsers, JSON.stringify(activeEmployeesList));
  }, [activeEmployeesList]);
  ...
  useEffect(
    () => {
      ...
      return () => window.removeEventListener('beforeunload', setActiveUsers);
    },
    [setActiveUsers],
  );
